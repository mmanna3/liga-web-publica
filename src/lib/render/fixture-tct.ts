...
function filaPartidoHtml(partido: FixturePartidoDTO): string {
  return `<div class="flex items-center gap-2 px-2 py-2.5 ${CLASE_FILA_TABLA_ESCUDO}">
    ${escudoImgHtml(partido.localEscudo, 'h-9 w-9', 'bg-zinc-200')}
    ...
    ${escudoImgHtml(partido.visitanteEscudo, 'h-9 w-9', 'bg-zinc-200')}
  </div>`;
}
